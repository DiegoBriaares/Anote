...
$iconSource = Join-Path $repositoryRoot "public\anote.svg"
...
& $bootstrapPython (Join-Path $controlCenterRoot "release\generate-icons.py") --source $iconSource --output $iconRoot
if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $iconFile -PathType Leaf)) { throw "Could not generate the native application icon." }
$metadata = @"
...
& $iscc "/DProductVersion=$packageVersion" "/DSourceDirectory=$appPath" "/DOutputDirectory=$distPath" "/DIconFile=$iconFile" (Join-Path $PSScriptRoot "control-center.iss")
