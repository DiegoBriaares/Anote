...
if [[ -f "$ANOTE_ENV_FILE" ]]; then
  ANOTE_SECRET="$(sed -n 's/^SECRET_KEY=//p' "$ANOTE_ENV_FILE" | head -1)"
fi
if [[ -z "$ANOTE_SECRET" ]]; then
  ANOTE_SECRET="$(openssl rand -hex 32)"
fi

umask 077
printf 'SECRET_KEY=%s\nANOTE_DATA_DIR=%s\nANOTE_IMAGE_TAG=%s\nANOTE_BIND_ADDRESS=%s\nANOTE_PUBLIC_PORT=%s\n' \
  "$ANOTE_SECRET" "$ANOTE_DATA_DIR" "$ANOTE_IMAGE_TAG" "$ANOTE_BIND_ADDRESS" "$ANOTE_PUBLIC_PORT" \
  > "$ANOTE_CANDIDATE_ENV"
