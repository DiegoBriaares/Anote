...
- **APP-GATE:** ESLint, the production TypeScript/Vite build and 30/30 test
  files with 120/120 tests passed on the review-corrected source.
  `server/backendHardening.test.ts` passed 23/23, including migration,
...
| DATA-006 Requested legacy dataset contains inconsistent private rows and duplicate program identities | Read-only inspection of the operator-selected `cal-ap` copy found 43 legacy event-note rows: 38 prove both event and same-owner role, three reference deleted events, and two have no provable same-owner role. Two user preferences also reuse one non-empty program ID. | Failing the whole transfer strands otherwise valid production data; deleting rows loses private content; fabricating events/roles or trusting role ownership reveals or misattributes it; nondeterministic collision repair makes release migration non-reproducible. | Partition every unprovable note losslessly into non-routable recovery storage, preserve SQLite value type and digest, treat event/role owners only as hints, assert row conservation, and derive collision IDs deterministically in stable user order. | **Closed.** Schema migration 3 preserves five unresolved rows without an HTTP/admin/log surface and migrates 38 valid rows; migration 4 produces two distinct deterministic program IDs while preserving unrelated preferences. Focused fixtures cover missing/foreign roles, typed values, replay and rollback; the requested copied dataset was reverified on schema 5 without changing its source. |
