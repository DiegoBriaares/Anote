# User Menu

**Component anchor:** `src/App.tsx`

The **User Menu** is the fixed avatar control in the top-right corner.

## Part Terms

- **Avatar Toggle**: The clickable avatar, username, and chevron that opens the menu.
- **Menu Panel**: The dropdown that contains account navigation and logout.
- **Profile Link**: Opens the Profile Page.
- **Roles Link**: Opens the Roles Page.
- **Friends Link**: Opens the Friends Page.
- **Admin Link**: Opens the Admin Page for admin users.
- **Logout Action**: Ends the session and returns to the Authentication Page.
